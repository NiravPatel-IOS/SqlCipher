...
+ (id)databaseWithName:(NSString*)dbName {
    ...
    aPath = [aPath stringByAppendingPathComponent:dbName];
    
    NSFileManager *ofmFileManager = [NSFileManager defaultManager];
    if(![ofmFileManager fileExistsAtPath:aPath]){
        NSError *oeError;
        BOOL bSuccess;
        NSString *osDefaultDBPath = [[[NSBundle mainBundle] resourcePath] stringByAppendingPathComponent:dbName];
        bSuccess = [ofmFileManager copyItemAtPath:osDefaultDBPath toPath:aPath error:&oeError];
        if (!bSuccess) {
            NSAssert1(0, @"Failed to create writable database file with message '%@'.", [oeError localizedDescription]);
        }
    }
    else{
...
- (BOOL) open {
    ...
    {
        const char*key = [[NSString stringWithFormat:@"PRAGMA key = temp"] UTF8String];
        sqlite3_exec(db, key, NULL, NULL, NULL);
...
-(void)createtable
{
    const char *sqlStatement = "CREATE TABLE IF NOT EXISTS USER (ID INTEGER PRIMARY KEY  NOT NULL  UNIQUE , USER_ID VARCHAR, USER_FNAME VARCHAR, USER_LNAME VARCHAR, USER_PHONE VARCHAR);"
    "CREATE TABLE IF NOT EXISTS USER2 (ID INTEGER PRIMARY KEY  NOT NULL  UNIQUE , USER_ID VARCHAR, USER_FNAME VARCHAR, USER_LNAME VARCHAR, USER_PHONE VARCHAR, USER_EMAIL VARCHAR, USER_AZOVA VARCHAR, USER_SEX VARCHAR, USER_AGE VARCHAR, USER_CAST VARCHAR);";
...
- (VIStatement*) cachedStatementForQuery:(NSString*)query {
    return [cachedStatements objectForKey:query];
}

- (void) setCachedStatement:(VIStatement*)statement forQuery:(NSString*)query {
...
- (BOOL) goodConnection {
    ...
    VIResultSet *rs = [self executeQuery:@"select name from sqlite_master where type='table'"];
...
- (void) compainAboutInUse {
    ////NSLog(@"The VIDatabase %@ is currently in use.", self);
    
    if (crashOnErrors) {
        NSAssert1(false, @"The VIDatabase %@ is currently in use.", self);
    }
}
...
- (id) executeQuery:(NSString *)sql arguments:(va_list)args {
    ...
    if (!statement) {
        statement = [[VIStatement alloc] init];
    ...
    rs = [VIResultSet resultSetWithStatement:statement usingParentDatabase:self];
    [rs setQuery:sql];
...
- (BOOL) executeUpdate:(NSString*)sql arguments:(va_list)args {
    ...
    if (shouldCacheStatements && !cachedStmt) {
        cachedStmt = [[VIStatement alloc] init];
...
@implementation VIStatement

- (void)dealloc {
	////NSLog(@"VIStatement Dealloc");
	[self close];
    [query release];
	[super dealloc];
}
